...
//
// Client identity for rate limiting is the TCP peer. Behind a reverse
// proxy, pass the proxy CIDRs via --trusted-proxy so X-Forwarded-For is
// honored; XFF is never trusted otherwise. Terminate TLS in front for
// public use.
func Rendezvous(args []string) int {
	...
	fs.Var(&proxies, "trusted-proxy", "proxy CIDR whose X-Forwarded-For is honored (repeatable)")
