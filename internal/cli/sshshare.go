...
// SSHShare is Catflap's new core command: grant a temporary SSH login
// to whoever holds the printed pairing code, for the given TTL, and
// nothing else. Unlike Share/Serve there is no policy, no capability
// file, no admin API, and no grant/tasks/revoke surface — an ephemeral
// SSH endpoint IS the product, and Ctrl-C (or the TTL) is how it ends.
//
// This coexists with the legacy capability-gateway commands (share,
// serve, grant, ...) while both stacks are live; it is not yet wired
// into cmd/catflap's dispatch under the "share" name.
func SSHShare(args []string) int {
	fs := flag.NewFlagSet("ssh-share", flag.ContinueOnError)
